...
namespace Commandy.Tests
{
    public class LinuxCommandTests
    {
        ...
        {
            var command = CommandProvider.CreateCommand("echo", opt => opt.UseShell().AddArgument("$TEST_ARG").AddEnvironmentVariable("TEST_ARG", "TEST_VALUE"));
            var result = await command.ExecuteAsync();
        ...
        public void RunCommand_WithShell_WhenCancel_ShouldExitCodeBeLessThanZeroAndContainsContentBeforeExit()
        {
            ...
            var command = CommandProvider.CreateCommand("echo.sh");
